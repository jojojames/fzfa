# fzfa fuzz tests

These tests exercise the boundaries where the recent fzfa bugs appeared. They
live under `fuzz/`, are not loaded by the package, and do not replace any fzfa
function.

[`CONTRACTS.md`](CONTRACTS.md) records the historical failure witnesses,
expected behavior, generator neighborhoods, and current coverage gaps.  A
contract remains partial until a controlled broken implementation makes its
oracle fail.

[`TRACE_FORMAT.md`](TRACE_FORMAT.md) defines the versioned case and failure
artifact format. Generators finish before drivers run. Exact replay consumes
the saved initial state and actions without consulting the random generator.

The targets are:

- `make compile`: byte-compile every fuzz harness and treat warnings as errors.
- `make selftest`: require generated producer traces to reach their intended
  race witnesses, then inject eight controlled defects and require the matching
  state oracle to reject each one.
- `make trace-selftest`: round-trip trace data and raw bytes, reject malformed
  files, prove replay does not call the generator, and reproduce one controlled
  failure by its stable signature.
- `make reduce-selftest`: reduce a controlled seven-action failure to its
  two-action witness, require the same signature, and prove a second reduction
  cannot remove another action.
- `make replay`: run small fixed regression cases.
- `make replay-trace TRACE=FILE`: exactly replay one saved state, native
  producer, or ugrep trace or failure artifact.
- `make replay-trace-live TRACE=FILE LIVE_EMACS_FLAGS=-nw`: exactly replay one
  live icomplete trace through a real minibuffer.
- `make reduce-trace TRACE=FILE [OUTPUT=FILE]`: remove actions from a saved
  non-live failure while it keeps reaching the same oracle signature.
- `make reduce-trace-live TRACE=FILE [OUTPUT=FILE]`: perform the same reduction
  through a real minibuffer; pass `LIVE_EMACS_FLAGS=-nw` in a terminal.
- `make state`: generate candidate-list mutations, late producer callbacks,
  restart/stop races, stale poll publications, and message ownership contexts.
- `make producer`: send generated bytes through a real fzfa command source and
  fzf-native 2.7. Cases include CRLF, ANSI escapes, invalid UTF-8, long lines,
  NUL bytes, nonzero process exits, a forced interim publication, and line-cap
  rows at N-1, N, and N+1.
- `make producer-selftest`: require every byte-stream category to be generated,
  then inject five controlled defects and require the producer and ugrep
  oracles to reject them.
- `make tools`: run the command built by `fzfa-ugrep` in a generated directory.
  It checks the documented Info and EMMS exclusions. If another late binary
  reaches the pipe, it checks that fzf-native rejects the NUL and fzfa reports
  the failure once.
- `make live`: drive a real icomplete-vertical minibuffer. Each next key waits
  for the prior query to render. The oracle checks initial growth, a smaller
  narrowed candidate set, a fresh full empty-query render, and advice cleanup.
  Its self-test replays one trace with random generation disabled. Three
  controlled canaries then disable fitting, disable filtering, or reuse stale
  empty-query candidates; all three must be rejected before generated cases run.

`state` uses a fake clock and timer queue, but it calls fzfa's real state
functions. `producer` uses the real native module and child processes. `live`
uses the real Emacs command loop. Together they cover different failure modes;
one target is not a substitute for the others.

## Run locally

The default directory layout is:

```text
parent/
  fzf-async/
  fzf-native/
```

From `fzf-async/fuzz`:

```sh
make selftest replay state
make trace-selftest
make reduce-selftest
make producer-selftest
make producer
make tools
```

If fzf-native is elsewhere, pass it explicitly:

```sh
make state FZF_NATIVE_DIR=/path/to/fzf-native
```

`producer` needs Python 3. `tools` skips with a clear message when `ugrep` is
not installed.

The live target needs a display or a terminal. In a local terminal, use:

```sh
make live LIVE_EMACS_FLAGS=-nw
```

CI runs terminal Emacs inside a pseudo-terminal supplied by `script`.
The state and live jobs cover Emacs 29.1, 30.1, and the current snapshot.

## Reproduce a failure

An oracle failure prints its stable signature, seed, generated trace, expected
and observed values, and the path of a versioned `.sexp` artifact. Live failures
include the sequence, query, candidate set, line count, and window height of
every observed render. A failure artifact is the preferred reproducer:

```sh
make replay-trace TRACE=/path/to/failure.sexp
make replay-trace-live TRACE=/path/to/live-failure.sexp LIVE_EMACS_FLAGS=-nw
```

Reduce a failure before turning it into a permanent regression:

```sh
make reduce-trace TRACE=/path/to/failure.sexp
make replay-trace TRACE=/path/to/failure.sexp.min.sexp
```

Pass `OUTPUT=/path/to/name.sexp` to choose the output path. The reducer never
overwrites its input. It first removes groups of actions, then tries each
remaining action until no single deletion preserves the signature. Invalid
traces, passing traces, and different failures do not count as successful
reductions. Candidate strings, bytes, delays, and initial configuration are
left unchanged by this phase.

Seed replay remains available for exploring neighboring generated cases:

```sh
FZFA_FUZZ_SEED=123 make state CASES=1
FZFA_FUZZ_SEED=123 make producer CASES=1
FZFA_FUZZ_SEED=123 make live LIVE_CASES=1 LIVE_EMACS_FLAGS=-nw
```

Useful controls are:

- `CASES`: number of generated state or producer cases.
- `STEPS`: operations in each producer-lifecycle state trace.
- `LIVE_CASES`: number of real minibuffer sessions.
- `FZFA_FUZZ_SEED`: first deterministic seed.

The state lane currently labels the process-buffer `fzfa--print` ownership
case as `KNOWN` when it occurs. It does not require that gap to remain: once the
production behavior is fixed, the same replay prints `RESOLVED` and continues.

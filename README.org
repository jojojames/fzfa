...
[[https://melpa.org/#/fzfa][file:https://melpa.org/packages/fzfa-badge.svg]]
[[https://stable.melpa.org/#/fzfa][file:https://stable.melpa.org/packages/fzfa-badge.svg]]

#+HTML: <img src="./screenshots/demo.webp" alt="fzfa demo" />
...
5. each command runs with an empty query to gather candidates,
   so they can be easily composed into a bigger multi-source command
...
Required dependency (~fzfa~ installs this automatically.):
[[https://github.com/dangduc/fzf-native][fzf-native 2.7]] (provides batch scoring
and the request-aware command-session API).
...
| ~fzfa-replay-from-memory~     | ~ivy-resume~                                    | ~vertico-repeat~?                   | ~helm-resume~                      |
...
| ~fzfa-regexp~            | ~swiper~ (regexp)  | ~consult-line~ (regexp) | ~helm-regexp~                  |

*** ~fzfa-regexp~

~fzfa-regexp~ scans the current buffer using an *Elisp* regexp and then
~fzf-native~ scores the resulting matches.

**** Two-phase semantics

The prompt is split on ~fzfa-separator~ (default ~::~):

#+begin_example
  REGEXP :: FUZZY-FILTER
#+end_example

- **CMD half (left)** — an Elisp regexp evaluated against the
  originating buffer.  Every matching line becomes a candidate
  annotated as ~LINE:COL: TEXT~, with the matched region underlined
  via ~fzfa-regexp-match~.  Empty CMD emits every line, so fzf-native
  becomes the primary filter.

- **FILTER half (right)** — passed to fzf-native for fuzzy re-scoring
  of the regexp-matched candidates.

Selecting a candidate jumps to the matched position in the source
buffer.

**** Capture groups → distinct colours

Capture groups in the CMD regexp paint each group's span with its own
face, layered on top of the base underline.  The default mapping is
~fzfa-regexp-group-faces~ — four faces inheriting from
~font-lock-keyword-face~, ~font-lock-function-name-face~,
~font-lock-string-face~, and ~font-lock-type-face~.  Theme switches
carry the colours along automatically; remap the variable to
substitute your own palette.

When the FILTER half also hits, ~completions-common-part~ from
fzf-native layers on top of the group + base faces — the fzf-native
default highlight handler preserves caller-attached faces end-to-end
rather than stripping them.

**** Examples

(Type these into the ~fzfa-regexp~ prompt.

#+begin_example
  defun ; defun is underlined.
#+end_example

#+begin_example
  defun \([a-z-]+\) ; Group 1 is colored.
#+end_example

#+begin_example
  (\(defun\|defvar\|defcustom\) \([a-z-]+\) ; 2 groups are highlighted.
#+end_example
...
#+begin_example
  (\(setf\) (\(fzfa-source-[a-z-]+\) \([a-z]+\)) \([a-z]+\)) ; 4 highlight groups
#+end_example
...
Demo below:
#+HTML: <img src="./screenshots/narrow.webp" alt="narrowing demo" />
...
On every changed query each source is re-filtered.  Sync sources use
~fzf-native-score-all~.  Async sources submit one request, poll metadata, and
materialize one authoritative snapshot through the request-aware session API.
Stable redraws reuse that materialized output.  The producer (for example
~find~, ~fd~, or ~ripgrep~) does not reset and continues generating candidates
for native incremental retries.

This lets one prompt combine, say, ~recentf~ entries (sync list) alongside a
streaming ~rg~ search (async producer) and have both groups score
against the same query as the user types:

* Replay

Every ~fzfa-completing-read~ / ~fzfa-multi-read~ session is captured
to an in-memory ring at exit — the source specs, the active narrow,
the typed filter, any in-session edits to the shell command, and the
display state.  ~fzfa-replay~-* commands let you pick a past
session and replay it as if you'd just invoked the original command.

Two rings: an in-memory list (current Emacs lifetime, recent
activity, capped by ~fzfa-sessions-max~), and an on-disk file
(across Emacs lifetimes capped by ~fzfa-replay-max-saved-items~).
 The disk ring is opt-in via ~fzfa-replay-mode~ — without it,
replay still works for the current (in-memory) Emacs session.

** Commands

| Command                   | Source ring                   |
|---------------------------+-------------------------------|
| ~fzfa-replay-from-memory~ | in-memory (current Emacs)     |
| ~fzfa-replay-from-file~   | on-disk (past Emacs sessions) |
| ~fzfa-replay-any~         | both, as multi-source         |

Note, this is a Replay, not a Resume (e.g. ~helm-resume~), it attempts to
replay a past ~fzfa-*~ command's inputs (e.g. command, directory, query, etc).

** Configuration

| Variable                         | What it controls                                           |
|----------------------------------+------------------------------------------------------------|
| ~fzfa-sessions-max~              | In-memory ring cap (default 16)                            |
| ~fzfa-sessions-exclude-commands~ | Commands to exclude from replay                            |
| ~fzfa-replay-file~               | On-disk path (default ~~/.emacs.d/cache/replay/fzfa-replay-<major>~) |
| ~fzfa-replay-max-saved-items~    | On-disk ring cap (default 20)                              |
| ~fzfa-replay-save-file-modes~    | Permission bits on the file (default ~#o600~ — owner-only) |
| ~fzfa-replay-auto-save-interval~ | Idle seconds between background saves (default 300)        |
| ~fzfa-replay-any-commands~       | Sources shown by ~fzfa-replay-any~ (default memory + file) |
...
| ~tramp~     | ~fzfa-tramp~     | ~tramp~ (built-in)                                  | Transparent TRAMP support: fzfa's shell commands (~fzfa-rg~, ~fzfa-fd~, …) work under a TRAMP ~default-directory~.  Also ships ~fzfa-ssh~ (pick an ~/.ssh/config host and visit ~/ssh:HOST:~) and ~fzfa-tramp~ (pick a host and open ~fzfa-browse-files~ rooted there).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ~transient~ | ~fzfa-transient~ | ~transient~ (built-in to Emacs 28+)                 | ~fzfa-transient~ — width-adaptive top-level menu over every ~fzfa~ command.  Sub-prefixes ~fzfa-transient-find~, ~-grep~, ~-vcs~, ~-project~, ~-search~, ~-shell~, ~-marks~, ~-code~, ~-org~, ~-mail~, ~-music~, ~-evil~, ~-chrome~, ~-firefox~, ~-safari~, ~-passwords~, ~-info~, ~-replay~ are autoloaded and bindable directly.  Find / grep / vcs sub-prefixes expose ~--dir=~ (current / project / default) and (for vcs) ~--backend=~ (smart / git / hg) infix args.  Layout is data-driven: each menu column is a defvar like ~fzfa-transient---multi~ — ~setq~ to customize, no transient-replace-suffix needed.  ~transient~ is loaded lazily on first invocation                                                                                                  |
...
(ag chrome company eglot emacs embark evil fd find firefox flymake git grep
 ...
 project regexp rg safari shell spotlight tramp transient ugrep vertico vc)
#+end_src
...
~fzfa-sync-autoloads~ walks ~fzfa--extension-registry~ and
...
After ~:init~ runs: ~M-x fzfa-firefox-~ no longer completes,
~(fboundp 'fzfa-firefox-bookmarks)~ returns ~nil~, every other
extension stays untouched.

* TRAMP support

TRAMP work transparently.  From =/ssh:host:/srv/=,
~fzfa-fd~, ~fzfa-rg~, ~fzfa-find~ etc. spawn the tool remotely
and open candidates through TRAMP.
...
#+HTML: <img src="./screenshots/rg-comparison.webp" alt="rg comparison" />
...
How to handle prefix args on ~fzfa-*~ commands is set up with ~fzfa-action-config~.

Defaults for the ~fzfa-file~ category (every file picker:
~fzfa-fd~, ~fzfa-find~, ~fzfa-rg-files~, ~fzfa-locate~, …):

| Prefix        | Directory                        | Action                   |
|---------------+----------------------------------+--------------------------|
| (none)        | ~fzfa--default-dir~              | ~fzfa-smart-find-file~   |
| ~C-u~         | ~fzfa--default-dir~              | ~find-file-other-window~ |
| ~C-u C-u~     | buffer ~default-directory~       | ~fzfa-smart-find-file~   |
| ~C-u C-u C-u~ | prompt via ~read-directory-name~ | ~fzfa-smart-find-file~   |
|               |                                  |                          |

For example:
~C-u~ ~fzfa-fd~ will use ~find-file-other-window~.
~C-u~ ~C-u~ will default to the current directory.
~C-u~ ~C-u~ ~C-u~ will prompt for a directory.

This is customizable, change ~fzfa-action-config~ to set up which actions/directories
are used for empty prefixes (default case) or ~C-u~ case.

Prefix handling is looked up by ~fzfa~ category.

Other categories:

- ~fzfa-buffer~ — ~fzfa-smart-switch-to-buffer~; ~C-u~ →
  ~switch-to-buffer-other-window~.
- ~fzfa-bookmark~ — ~fzfa-smart-bookmark-jump~; ~C-u~ →
  ~bookmark-jump-other-window~.

To customize a single entry without setting the entire alist:

#+begin_src emacs-lisp :tangle yes
  ;; Make C-u C-u C-u open at project root instead of prompting.
  (setf (alist-get '(64)
                   (alist-get 'fzfa-file fzfa-action-config)
                   nil nil #'equal)
        `(:directory ,(lambda ()
...
For ~async~ sources (e.g. ~fzfa-find~, ~fzfa-rg~, etc), on an empty input,
candidates are not sorted and come in whatever order the shell command produced.

With input/query, candidates are filtered and sorted according to ~fzf-native~.

With ~sync~ sources (e.g. ~fzfa-M-x~, ~fzfa-buffer~, etc), on an empty input,
candidates are sorted according to recency (history), and then length.

With input/query, candidates are first scored/sorted by ~fzf-native~. Tie breaks
(common when input string is short) are then sorted by recency (history), and
then length. This means when the query is short, recency has high weight, and
as the query gets longer, ~fzf~'s algorithm begins to show.
...
(#("visual-line-mode" ; frequently used
   0 1 (face completions-common-part completion-score 255)
   1 4 (face completions-common-part)
   5 6 (face completions-common-part)
   7 11 (face completions-common-part)
   12 14 (face completions-common-part))
 #("global-visual-line-mode"
   0 1 (completion-score 255)
   7 11 (face completions-common-part)
   12 13 (face completions-common-part)
   14 18 (face completions-common-part)
   19 21 (face completions-common-part))
 #("menu-bar--visual-line-mode-enable"
   0 1 (completion-score 255)
   10 14 (face completions-common-part)
   15 16 (face completions-common-part)
   17 21 (face completions-common-part)
   22 24 (face completions-common-part)))
#+end_src

Without the history / length tiebreak, "visual-line-mode" could be sorted
...
#+HTML: <img src="./screenshots/vertico-columns.webp" alt="vertico columns" />
...
#+HTML: <img src="./screenshots/ivy.webp" alt="ivy transformer" />
...
#+HTML: <img src="./screenshots/helm.webp" alt="helm dispatch" />
...
For ~:command~ sources a polling timer checks the session's
~:snapshot-generation~ through ~fzf-native-async-status~ and calls
~helm-force-update~ when a completed result is published.  The multi handler
uses a single shared timer that watches every source's handle, throttled to one
...
** ~minibuffer~ (Emacs 31+ eager display)

#+HTML: <img src="./screenshots/minibuffer.png" alt="fzfa under Emacs's built-in *Completions* frontend" />

Config I use when running ~fzfa~ against the built-in minibuffer:
...
Rendering the ~*Completions*~ window is proportional to how many
candidates we return, so ~fzfa~ applies a separate cap for this
frontend via ~fzfa-default-minibuffer-max-candidates~ (default
~10000~).
** ~transient~
A few transients are included in the ~transient~ extension (~fzfa-transient.el~).
~M-x fzfa-transient~ opens a width-adaptive top-level menu over every
~fzfa~ command; the layout switches between wide and narrow
arrangements based on the frame's share of the display width.

Individual transients are autoloaded and bindable directly:

#+begin_src emacs-lisp :tangle yes
  (keymap-global-set "C-c f"   #'fzfa-transient)
  (keymap-global-set "C-c f g" #'fzfa-transient-grep)
  (keymap-global-set "C-c f v" #'fzfa-transient-vcs)
#+end_src

The ~find~, ~grep~, and ~vcs~ transients expose infix args:

- ~--dir=~ — ~current~ (~default-directory~), ~project~
  (~project-root~ of the current project), or ~default~ (fzfa's
  default directory resolution).  The selection is read by the
  suffix dispatcher and used to ~let~-bind ~fzfa-directory~ around
  the actual ~fzfa~ command.
- ~--backend=~ (vcs only) — ~smart~ (dispatch via ~vc-responsible-backend~),
  ~git~ (force ~fzfa-git-*~), or ~hg~ (force ~fzfa-hg-*~).
...
** Telescope-style layout (~fzfa-posframe-mode~)
Inspired by: [[https://github.com/nvim-telescope/telescope-fzf-native.nvim][telescope]]

~posframe~ is required for this feature to work. Install separately. This feature
isn't enabled by default so ~fzfa-posframe-mode~ needs to be called manually.

#+BEGIN_SRC emacs-lisp
;; fzfa-preview-delay is nil by default which disables automatic previews.
(setq fzfa-preview-delay 0.25)
(fzfa-posframe-mode 1)
#+END_SRC

#+BEGIN_SRC emacs-lisp
;; By default posframe sets up the preview frame centered above.
;; Set to side-by-side to get a telescope-esque layout.
(setq fzfa-posframe-layout 'side-by-side)
#+END_SRC

~fzfa-posframe-mode~ works with ~vertico~, ~helm~, and ~ivy~.

~ivy~ requires an additional package: [[https://github.com/tumashu/ivy-posframe][ivy-posframe]]
...
*** Media thumbnails inside the preview
We can view media thumbnails inside ~dired~ inside the preview like so:
...
(use-package media-thumbnail
  ...
  (add-hook 'dired-mode-hook
            (lambda ()
              (when (bound-and-true-p fzfa-loading-preview)
                (media-thumbnail-dired-mode))))
  :config
  (setq media-thumbnail-image-width 300))
#+END_SRC

#+HTML: <img src="./screenshots/telescope.webp" alt="telescope-style layout" />
...
| ~fzfa-preview-delay~           | ~nil~                    | Idle seconds before auto-preview fires (nil = manual only; see note below).   |
| ~fzfa-preview-key~             | ~C-c C-p~                | Key bound to ~fzfa-preview-current~ in fzfa sessions (nil disables).          |
...
Preview fires for commands that act on the highlighted candidate
(e.g. ~fzfa-theme~ loading themes, ~fzfa-grep~ jumping to the matching
line in a side window).  Two knobs control when it fires:

- ~fzfa-preview-key~ (default ~C-c C-p~) — press in a fzfa session to
  preview the current candidate on demand.
- ~fzfa-preview-delay~ (default ~nil~) — when set to a number, also
  fires preview automatically after that many idle seconds.  Uses
  ~run-with-idle-timer~ so fast typing / arrow-key bursts suppress
  intermediate previews; at fire time the *current* candidate is
  re-read (never a stale one captured at schedule time).  ~0~ previews
  immediately on every selection change.

Set both to nil to disable preview entirely.
...
A 2-arg producer example refires per CMD-half change.  See
